...
export class ImageController {
    ...
    async createContainer(@Param('id') name:string){
        ...
        console.log("Container (" + containerName + ") was created successfully. requestId: ", createContainerResponse.requestId);
        return "Container (" + containerName + ") was created successfully. requestId: "+ createContainerResponse.requestId;
    }
    ...
    async getAllBlobs(@Param('id') name:string){
        ...
        for await (const blob of containerClient.listBlobsFlat()) {
            ...
            var link = 'https://' + accountName + '.blob.core.windows.net/' + name + '/'+ blob.name +'';
    ...
    async deleteContainer(@Param('id') name:string){
        ...
        return "Container (" + containerClient.containerName + ") deleted successfully. requestId: "+ deleteContainerResponse.requestId;
    }
    ...
    async upload(@UploadedFile() file, @Param('id') name:string) {
        ...
        try {
            const blobName = file.originalname.substr(0, file.originalname.lastIndexOf('.')) + '-' + file.filename;
